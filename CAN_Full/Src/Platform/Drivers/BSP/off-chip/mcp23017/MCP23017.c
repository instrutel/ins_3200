...
static HAL_StatusTypeDef MCP23017_RegisterRead (I2C_HandleTypeDef *hi2c,uint8_t SlaveAddr,uint8_t RegAddr,uint8_t *ReValue);
...
//				Dev_Mcp2307PortRegRead(salveAddr,port,&data);
//				Mcp23017_Status = MCP23017_RegisterWrite(salveAddr,DEFVALA,config->DefVal);
			break;
		case MCP_PORT_B:
...
//				Dev_Mcp2307PortRegRead(salveAddr,port,&data);
//				Mcp23017_Status = MCP23017_RegisterWrite(salveAddr,DEFVALB,config->DefVal);
			break;
	}
	return 0;
...
uint8_t MCP23017_PortPinRead(I2C_HandleTypeDef *hi2c,uint8_t SlaveAddr,MCPPorts_e port,uint8_t pin, uint8_t* pValue)
{
	...
	Status_l = MCP23017_RegisterRead(SlaveAddr,REG_ADDR,&Port_Data);
...
uint8_t MCP23017_PortPinWrite(I2C_HandleTypeDef *hi2c,uint8_t SlaveAddr,MCPPorts_e port,uint8_t Pin, uint8_t pValue)
{
	...
	/* Port register Value */
	return MCP23017_RegisterWrite(SlaveAddr,REG_ADDR,Port_Data);
}
...
uint8_t MCP23017_PortRegWrite(I2C_HandleTypeDef *hi2c,uint8_t SalveAddr,MCPPorts_e port,uint8_t pValue)
{
	...
	/* Port register Value */
	return MCP23017_RegisterWrite(SalveAddr,RegAddr,pValue);
}
...
uint8_t MCP23017_PortRegRead(I2C_HandleTypeDef *hi2c,uint8_t SalveAddr,MCPPorts_e port,uint8_t* pdata)
{
	/* Port A/B register Value */
	uint8_t RegAddr = (port == MCP_PORT_A)?GPIO_A:GPIO_B;
	return MCP23017_RegisterRead(SalveAddr,RegAddr,pdata);
}
...
static HAL_StatusTypeDef MCP23017_RegisterRead (I2C_HandleTypeDef *hi2c,uint8_t SlaveAddr, uint8_t RegAddr,uint8_t *ReValue)
{
	...
	{
		SlaveAddr = ( MCP23017_ADDRESS | SlaveAddr ) << 1;
		ret = HAL_I2C_Mem_Read(hi2c, SlaveAddr, RegAddr, I2C_MEMADD_SIZE_8BIT, ReValue, 0x01, 0xffffff);//- Default IOEXPCARD PORT Configuration
	}
	return ret;
}
