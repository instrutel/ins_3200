/**
  ******************************************************************************
  * @file    LwIP/udp_server.c
  * @author  Application Team
  * @version V1.3.1
  * @date    09-October-2024
  * @brief   UDP server/echo client
  ******************************************************************************
  * @attention
  *
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define UDP_SERVER_PORT    23

static uint16_t UDPClinetPort = 9999; /* define the UDP remote connection port */
...
void udp_server_receive_callback(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	if( hupcb == NULL || hupcb != upcb ) {
		...
		udp_connect(upcb, addr, UDPClinetPort);
		hupcb =  upcb;
		udp_server_send((uint8_t*)"Connected..!",12);
	}
...
uint8_t udp_client_connect(void)
{
  ...
  if (hupcb !=NULL)
  {
    ...
    err= udp_connect(hupcb, (const ip_addr_t*)&DestIPaddr, UDP_SERVER_PORT);
