...
/* @brief  Initialize the MCP23017 I/O Expander device.
 * @details This function initializes the MCP23017 I/O expander over the I2C bus.
 *          It configures the necessary device registers (e.g., IODIR, IPOL, etc.)
 *          to set the I/O directions, pull-ups, or interrupt behavior as required
 *          by the application.
 * @param  hi2c Pointer to the I2C handle (HAL I2C handle structure).
 * @param  i2c  Pointer to the I2C peripheral instance (optional, for low-level access if needed).
 * @retval uint8_t Returns 0 if initialization is successful, or an error code if failed.
 * @note   Ensure that the I2C peripheral is already initialized before calling this function.
*/
