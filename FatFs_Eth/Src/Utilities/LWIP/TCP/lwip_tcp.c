...
int8_t tcp_client_connect(uint16_t port, void (*func)(uint8_t *data, uint16_t Len))
{
  ...
  if (client_pcb != NULL)
  {
	...
	err = tcp_bind(client_pcb, IP_ADDR_ANY, 0);
    IP4_ADDR( &DestIPaddr, DEST_IP_ADDR0, DEST_IP_ADDR1, DEST_IP_ADDR2, DEST_IP_ADDR3 );

    if(es == NULL) {  /* allocate structure es to maintain tcp connection informations */
...
{
	err_t err = tcp_write(client_pcb, pData, len, TCP_WRITE_FLAG_COPY);
	/* tcp pcb out */
...
{
	/* close connection */
	tcp_client_connection_close(client_pcb, es);
}
...
static err_t tcp_client_connected(void *arg, struct tcp_pcb *tpcb, err_t err)
{
	...
	if (es != NULL)
	{
		if (err == ERR_OK)
		{
			...
			tcp_setprio(tpcb, TCP_PRIO_MIN+1);
...
static err_t tcp_client_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
	...
	{
		tcp_abort(tpcb);
		/* remote client abort the connection */
		es->state = ES_CLOSING;
		tcp_client_close();
		ret_err =  ERR_ABRT;
	}
...
int8_t tcp_server_send(uint8_t *ptr,uint16_t Len)
{
	...
	{
		/* send udp data */
		wr_err = tcp_write(es->pcb, ptr, Len, 1);
